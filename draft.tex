...
\end{equation}
\re{do we define $\langle \rangle$? Also -- should we explain in a sentence WHY the form of the equation changes when we go to period b.c.s?}
...
The default timestep in  \PyUltraLight  is fixed with reference to the fluid interpretation of the Schr{\"o}dinger-Poisson system \re{CITATION}. We obtain this via
%
\begin{equation}\label{eq:madelung}
    \psi\equiv\sqrt{\rho}e^{i\theta}, \quad \vec{v}\equiv\boldsymbol{\nabla}\theta
\end{equation}
%
\re{Do we need a bit more detail, or a reference?}  If the phase change between two adjacent grid points across a timestep, exceeds $\pi$ the fluid will appear to move backwards across the grid, and we set the default timestep, $\Delta t$, such that fluid travelling at this maximum velocity will traverse one grid space, $\Delta x$, per timestep, or
\begin{equation}\label{eq:timestep}
    \Delta t = \frac{(\Delta x)^2}{\pi}.
\end{equation}
%
This is  a choice, rather than a strict constraint but if the ``fluid'' approaches velocities where the phase appears to switch direction, the configuration will also be approaching the point where the simulation grid is too coarse to fully resolve the dynamics. Making the timestep much smaller than this value may offer little practical advantage. Conversely, in some cases the breakdown may occur in regions of the simulation volume of little physical interest, and the user is free to choose a larger timestep  via the 'step\textunderscore factor' parameter in the Jupyter notebook.

Alternatively, Ref.~\cite{Mocz:2017wlg} fixes the timestep by ensuring that neither of the unitary operators in Equation \ref{eq:schematic-psi}  leads to a phase change of more than $\pi$ for a single grid point over one timestep. However, because the pseudo-spectral algorithm does not compare the phase of a single gridpoint at different points in time, this choice of timestep is  not a requirement for stability. This method  give the following constraints: 
\begin{equation}
    \Delta t < \left[\frac{2\pi}{\Phi_{max}}, \frac{2(\Delta x)^2}{\pi}\right],
\end{equation}
%
where the second of these constraints is generally the stricter of the two, and is equivalent to the default choice up to a multiplicative factor of $\mathcal{O}(1)$. Our experience is that specifying the timestep via Equation \ref{eq:timestep} is suitable for the majority of simulation scenarios, and we explore convergence in more detail in Section \ref{sec:resolution}.
...
To examine the sensitivity of the  $\psi$ field configuration to the spatial resolution, we first run at $256^3$ with the default timestep. We then re-run  at resolutions  from $64^3$ to $320^3$ with the timestep fixed to the  $256^3$ value and downsample the final outputs to $64^3$. We  sort the resulting values by  the density at the corresponding spatial location, and plot differences in the phase and the magnitude of $\psi$ relative to the values of the $320^3$ run in Figures \ref{fig:spatial_magnitudes} and \ref{fig:spatial_phases}. \re{maybe this could be one combined figure with both plots.}  The convergence is poor  at $\psi$ at $64^3$, but improves with resolution, to the point that there is little difference between the  $256^3$ and $320^3$ cases.
...
To examine the sensitivity of the the  $\psi$ field configuration to the timestep, we take the same default simulation at $256^3$, and then compare this to runs with timesteps 0.1, 10, and 50 times the default and down-sample the final output arrays to $64^3$. We sort sort the array values in order of the $\psi$ field magnitude in the run with the smallest timestep and in Figures \ref{fig:temporal_magnitudes} and \ref{fig:temporal_phases} \re{maybe also combined into one full page figure?} we show the difference in the phase and magnitude of $\psi$ as a function of the timestep.  The difference between the results with the default timestep and a value 10 times smaller are negligible; and there is reasonable agreement between the default case and those with the timestep boosted by a factor of 10. However, when the timestep is increased by a factor of $50$ the accuracy of both the phase and magnitude data are significantly reduced.

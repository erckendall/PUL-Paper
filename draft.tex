...
The existence of an extremely light scalar field, minimally coupled to gravity, is the central premise on which ULDM models are predicated. Within the ULDM framework, it is proposed that cosmological structure formation proceeds through the Bose-Einstein condensation of this scalar field \sh{I don't understand this sentence. There is a Bose-Einstein condensate and it clusters to form structures, but the sentence seems to suggest that the clustering and condensating are the same thing?}. In this Section  \re{I would normally give Section a capital letter in this context} \ek{I would be inclined to only capitalise `section' in instances where it appears as a proper noun, i.e. "In Section X we showed...", where we are referring to a specifically named section, numbered X. It seems to me that in this particular context `section' is a common noun. Happy to capitalise all instances of the word if this is your recommendation.} 
...
\end{equation}
We have thus shown that $\psi$ satisfies the Schr{\"o}dinger equation in this limit, which is interpreted as the unique macroscopic wavefunction of a Bose-Einstein condensate \sh{I'm also not sure I understand this sentence. There are other equations that describe other Bose-Einstein condensates (e.g. other N-L Schrod equations with different potentials.) I suggest just removing the word unique.}. It follows that the particle \sh{suggest include 'number' after particle} density of the condensate is given by $\vert\psi\vert^2$, so its mass density is simply $m\vert\psi\vert^2$. The local gravitational potential thus satisfies the Poisson equation,
...
It is helpful to recast the Schr{\"o}dinger-Poisson system (equations \ref{eq:schrodinger} and \ref{eq:poisson}) in terms of adimensional quantities. In keeping with Refs~\cite{Schive2014,Paredes2016} we introduce length, time, and mass scales as follows\sh{I think I fixed a typo in equation 2.8, but please check. I added the 1/2 in the exponent on the RHS}:
\begin{align}
    \CMcal{L}&=\left(\frac{8\pi\hbar^2}{3 m^2H_0^2\Omega_{m_0}}\right)^{\frac{1}{4}}\approx121\left(\frac{10^{-23}\operatorname{eV}}{m}\right)^{1/2}\operatorname{kpc},\label{eq:length}\\
...
\end{equation}
where $\mathcal{T}$ is the time-ordering symbol \sh{I think I need this explained to me. I might just have a hole in my maths knowledge regarding what the $\mathcal{T}$ means in this context. I'm happy to go read up and fill that hole though so a reference might be enough.}. For a sufficiently small timestep $h$,  the trapezoidal rule gives the approximation
\begin{equation}
...
In many finite-differencing schemes for  partial differential equations the Courant-Friedrichs-Lewy (CFL) condition is a necessary condition for convergence \cite{Ajaib2013}, and puts an upper bound on the timestep as a function of the gridsize. The physical origin of the CFL condition is the finite speed of propagating information  in the evolving system \sh{split one sentence into two}.  Consequently, this condition is often cited in numerical analysis of ULDM physics governed by the Schr{\"o}dinger-Poisson system, see e.g. Ref.~\cite{Schwabe2016}. Strictly speaking, however, the CFL condition is applicable to codes using finite differencing methods for spatial derivatives, and is thus not applicable to pseudospectral codes like \PyUltraLight.   We also note that the Poisson equation is acausal in that it responds globally and instantaneously to local changes in the matter distribution. However, the Schr{\"o}dinger-Poisson equation is an approximation derived  in the nonrelativistic limit so this is not a shortcoming in regimes where the underlying equations are physically valid.
...
\begin{figure}
  ...
  \caption{Solitonic core after one revolution around a central potential, demonstrating the internal velocity field of an irrotational Riemann-S ellipsoid (where the collective motion of the soliton has been subtracted from the velocity field). Deformation of the spherical density profile into that of an ellipsoid can be seen, with extension along the radial direction (red arrow). The host:satellite mass ratio is approximately 55; simulation resolution is $256^3$. \sh{I assume the contours are density, but this is never explicitly stated.}}
  \label{fig:riemann}
\end{figure}

\PyUltraLight allows the inclusion of a static potential equivalent to a point-mass  at the centre of the simulation region.  There is no backreaction on this mass as a result of the ULDM dynamics, and its ``mirror images'' within the periodic coordinate systems are not accounted for within the overall gravitational potential. The setup is thus somewhat unphysical but it provides a starting point for a study of the stability of satellite dark matter halos orbiting a much larger object \sh{I'm a bit confused by the use of the word "unphysical" here, is this not more physical than an infinite array of point masses? A 1/r potential, which we have, is the *more* physical potential}. In particular, this includes the investigation of lifespans of dwarf satellite galaxies orbiting much larger objects (including the Milky Way) which are a key to understanding whether ULDM models can resolve the so-called missing satellites problem \cite{Weinberg2015}.

An extensive study of the tidal disruption of ULDM solitonic cores orbiting a central potential has recently been undertaken in \cite{Du2018} and we reproduce just one of their results here. When the Schr{\"o}dinger-Poisson system is recast as a system of hydrodynamical equations, the fluid velocity  at any point is the gradient of the smoothly-varying phase\sh{This correspondence is only approximate though, right? So the thing we call 'fluid velocity' must have zero curl, but the actual movement of mass doesn't necessarily need to be?}.  Since the curl of a gradient vanishes identically, the velocity field of the Schr{\"o}dinger-Poisson system must be irrotational, and $\nabla\times\vec{v}=0$. If a radially symmetric soliton is initialised in a circular orbit around a Newtonian potential, there will be initial transient behaviour as the spherical profile becomes elongated along the radial direction of the central potential. Meanwhile, the velocity field corresponding to the overall orbital motion of the soliton is superposed with the internal velocity field, combining so as to produce a net flow with vanishing curl. 

The family of Riemann-S ellipsoids describe uniformly rotating bodies which satisfy the condition of irrotationality \cite{Chandrasekhar1965} \sh{this sentence is really confusing, but I'm not sure of how to re-phrase it to make it less confusing so we might need to just accept the "rotating bodies... that are irrotational" confusingness}. Therefore we expect the characteristic internal velocity field of such an ellipsoid to arise during our simulations \sh{I split the previous sentence in two and re-ordered what is now the second sentence}. It is found in \cite{Du2018} that an initially spherical solitonic core without self-rotation will gradually spin up to form a tidally-locked irrotational Riemann-S ellipsoid when orbiting a host mass. We reproduce this result using \PyUltraLight. Figure \ref{fig:riemann} showing the internal velocity field of a solitonic satellite after one complete revolution around a host mass \sh{I spluit one sentence into two}. The soliton has become elongated along the radial line connecting it to the host, indicating that it is tidally locked, while the internal velocity field qualitatively matches that of an irrotational Riemann-S ellipsoid within the tidal radius \cite{Daller2012} \sh{This is stated as if the reader should be able to tell, but I would guess most of our readers haven't heard of a Riemann-S ellipsoid. I would suggest at least saying "see figure 2 of ref [37]", and perhaps even some more substantial description of hwat one expects a riemann s ellipsoid to look like. alternatively, we could not use that jargon and just appeal to the fact that the velocity field in our figure doesn't look rotational.}. 

It should be noted that the wider velocity field is not expected to be accurately predicted in a simulation of this kind. This is because the initial soliton density profile is defined only out to a given cutoff radius, beyond which the $\psi$ field value, and by extension its velocity field, is set identically to zero. Therefore, the velocity field far from the solitonic centre is not well-modelled in these simulations, and it may be expected that non-trivial self-interaction in the wider field would lead to unpredictable behaviour in these regions. \sh{This last paragraph was previously part of the preceding one. I found it a bit confusing because it reads like a defence of an anomaly in our own figure. I was going to suggest removing it but I think I now understand that it was put in as a warning to people who might use our code and get confused about the velocity field far from the centre of the solitons. We could either make that more clear or just hope that making it a separate paragraph does this implicitly}
...
\begin{figure}
  ...
  \caption{Evolution of the energy for a binary soliton system with each soliton in an elliptical orbit around the common centre of mass.\sh{Should probably add what energy code units are here too as people might not know to look at other figure}}
  \label{fig:binary}
\end{figure}
...
The two terms in the integral \ref{eq:energy-tot} are calculated separately within the code. The first term is the gravitational potential energy of the Schr{\"o}dinger-Poisson system, $E_{GP}$ \sh{I added $E_{GP}$ here because it is labelled like this in the figures and this makes it easier for people looking at the figures to find out what that quantity is. same for $E_K$ and $E_Q$ below}. As discussed in \cite{Hui2016}, the second term may be decomposed into contributions which may be considered separately as kinetic and `quantum' energies, $E_K$ and $E_Q$. However, for our purposes it is sufficient to consider only their combined contribution\sh{split one sentence into three}. When \PyUltraLight includes the central potential of a point mass located at the centre of the simulation grid we have additional energy contributions and calculate the  gravitational potential energy from self-interactions separately from the gravitational potential energy due to the central potential.

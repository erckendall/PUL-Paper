...
\ek{With regard to the `SP' vs. Schr{\"o}dinger-Poisson issue: I also don't like too many initialisms in publications but felt that in this case, due to the sheer number of instances of the term, and given that it seems to have been adopted as a standard by some of the other ULDM folk (e.g. arXiv 1801.03507, 1608.00802, 1801.04864), it made sense to compactify things. I've removed all instances of `SP' in this version, but feel that somehow the full term can appear rather cumbersome at times. What do you think?}
\re{Fair enough, but I think it reads better without -- ULDM works via analogy with CDM; it's not consistent but seems better to me. Also, have added and "EK" comment :-) }
...
\re{I agree, but thought ``this Section'' was an example of it being used as a proper noun. But it is not a huge deal either way.}
...
\begin{figure}
  ...
  \caption{Head-on collisions between two solitons of mass 5, relative velocity 6, and initial separation 4 in code units. Time progresses from left to right across each row. \re{maybe a few more words on what is going on (e.g. we see that in the lower panel the solitons slow as they approach one another. Also, should we choose masses/separations where it does not take  20+ Gyr for them to approach one another. As a test of the dynamics it is not a big deal, but it looks a bit funny if it is several times the age of the universe}}
  \label{fig:repulsion}
\end{figure}

As demonstrated in \cite{Paredes2016}, the wavelike properties of ULDM can give rise to effective forces which  dramatically affect the dynamics of core collisions. These effective forces arise as a result of interference phenomena, rather than because of any local interactions the ULDM model might  incorporate.  Figure \ref{fig:repulsion} shows the results of two \PyUltraLight where two solitons of equal mass undergo head-on collisions. Contours of the density profile along the plane of symmetry are displayed. In one case (top) there is no phase offset between the initial solitons, while in the second  the phases differ by $\pi$. In the latter case, the phase shift creates an effective repulsive force between the two solitons.  Further discussion of this phenomenon and its possible observational consequences can be found in \cite{Paredes2016}.
...
\re{Check the bib options so we don't see all the Planck authors :-) }

...
Having demonstrated improving energy conservation with respect to spatial resolution, we now examine the convergence of the $\psi$ field configuration as spatial resolution and timestep are augmented. To perform this analysis, we initialise a simulation of two solitons diametrically opposed about a large Newtonian central potential. We run the simulation forward until the solitons become significantly tidally disrupted in their orbits, as shown in Figure \ref{fig:disruption}. 
...
------------------------------
%Include figure with first and last configuration of the simulation, caption with parameters used

%Mention that tim,estep 1 not much better than timestep 0.1 look at phase
%Phase appears to be more sensitive than density to resolution
-----------------------------

To examine the sensitivity of the final $\psi$ field configuration to changes in spatial resolution, we first run this simulation at $256^3$, with the default timestep. We then re-run the simulation at resolutions ranging from $64^3$ to $320^3$, fixing the timestep in each case to be the same as the $256^3$ run. The final output arrays are then down-sampled such that we obtain five arrays of dimension $64^3$ for comparison. We arrange the array values in order of increasing density, and then analyse the deviation of the phase and the magnitude of $\psi$ from the values of the highest resolution run. The results of this process are shown in Figures \ref{fig:spatial_magnitudes} and \ref{fig:spatial_phases}.

\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 0 0 0,clip]{spatial_discretisation}
...
\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 0 0 0,clip]{phases}
...
To examine the sensitivity of the final $\psi$ field configuration to the choice of the timestep, we run the simulation at $256^3$, choosing the timestep to be 0.1, 1, 10, and 50 times the default timestep. Again we down-sample the output arrays to $64^3$, and arrange the array values in order of $\psi$ field magnitude. We then examine how the phase and magnitude of the field values differ in comparison to the case of 0.1 $\times$ the default timestep. The results of this analysis are shown in Figures \ref{fig:temporal_magnitudes} and \ref{fig:temporal_phases}. 

\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 0 0 0,clip]{temporal_discretisation}
  \caption{TBW}
  \label{fig:temporal_magnitudes}
\end{figure}

\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 0 0 0,clip]{temporal_phases}
  \caption{TBW}

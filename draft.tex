...
\end{equation}
%
where $\psi(\vec{x},t_{i})$ is the field configuration at this halfway point in the full timestep.\footnote{Unlike Equation \ref{eq:poisson_periodic}, in Equation \ref{eq:schematic-phi} the global average value of the density is not subtracted from the local value. This is because subtraction of a constant affects only the $k=0$ Fourier mode, which is manually set to zero within the code prior to the final inverse Fourier transform. Therefore, the subtraction of the average density has no effect on the final result, and is consequently not incorporated into the algorithm.} The final operation in equation \ref{eq:schematic-psi}  only changes the phase of  $\psi$, so we could replace $\psi(\vec{x},t_{i})$ with $\psi(\vec{x},t\hspace*{-.2em}+\hspace*{-.2em}h)$ in equation \ref{eq:schematic-phi} with no change in meaning. \PyUltraLight makes an additional simplification to the symmetrised split-step Fourier method by combining the consecutive half-steps in the nonlinear term into a single full step. Consequently, only the first and last operations involve actual half steps. Schematically this becomes 
%
\begin{equation}
...
When employing the method of finite differences to numerically solve systems of partial differential equations, the Courant-Friedrichs-Lewy (CFL) condition is a necessary condition for convergence \cite{Ajaib:2013oua}. This condition puts an upper bound on the size of the timestep as a function of the grid spacing. Consequently, this condition is often cited in numerical analysis of ULDM physics governed by the Schr{\"o}dinger-Poisson system, see e.g. Ref.~\cite{Schwabe:2016rze}. 

However, because \PyUltraLight uses pseudospectral methods to compute spatial derivatives, the algorithm is unconditionally stable \cite{Taha:1984jz} and timesteps need not be subjected to a CFL-like constraint. Consequently, the choice of timestep is in principle arbitrary, and is generally selected in accordance with accuracy requirements (our split-step algorithm is accurate to second order in the timestep).

While the user is encouraged to choose their own timestep and validate it through convergence testing on a case-by-case basis, \PyUltraLight does include a default timestep value which is chosen based on maximum fluid velocity arguments. In the fluid interpretation of the Schr{\"o}dinger-Poisson system, we define
\begin{equation}\label{eq:madelung}
    \psi\equiv\sqrt{\rho}e^{i\theta}, \quad \vec{v}\equiv\boldsymbol{\nabla}\theta.
\end{equation}
This then allows us to define a maximum fluid velocity that can be successfully modelled on a simulation grid of a given resolution. If the phase change between two consecutive grid points exceeds $\pi$, the fluid will appear to move backwards across the grid. Hence, we define the maximum velocity as that corresponding to a phase change of $\pi$ between grid points. We then set the default timestep, $\Delta t$, such that fluid travelling at this maximum velocity will traverse one grid space, $\Delta x$, per timestep. That is to say
\begin{equation}\label{eq:timestep}
    \Delta t = \frac{(\Delta x)^2}{\pi}.
\end{equation}
We stress that this is merely a choice of timestep, rather than a strict constraint. The motivation for this particular choice of timestep is that if the user finds that their fluid is approaching velocities at which this timestep is no longer sufficiently small for convergence, they will also be approaching the point at which they need to refine the simulation grid. Therefore, choosing the timestep to be smaller than the above default value offers little advantage. In many cases, this choice of timestep will be overly strict, and the user is free to choose a larger timestep by modifying the 'step\textunderscore factor' parameter in the Jupyter notebook.

An alternative method of choosing the timestep, employed for example in \cite{Mocz:2017wlg}, is to ensure that neither of the unitary operators in Equation \ref{eq:schematic-psi} can lead to a phase change of more than $\pi$ for a single grid point over one timestep. However, because the pseudo-spectral algorithm does not compare the phase of a single gridpoint at different points in time, this choice of timestep is also not a requirement for stability. Using this method of choosing the timestep, we find the following set of constraints: 
\begin{equation}
    \Delta t < \left[\frac{2\pi}{\Phi_{max}}, \frac{2(\Delta x)^2}{\pi}\right],
\end{equation}
where the second of these constraints is generally the stricter of the two, and is equivalent to our previous choice of timestep up to a multiplicative factor of $\mathcal{O}(1)$.

Convergence testing has demonstrated that choosing the timestep according to Equation \ref{eq:timestep} is suitable for the majority of simulation scenarios. This is demonstrated in section \ref{sec:resolution}, where we contrast the performance of \PyUltraLight for differing choices of timestep.
...
\section{Convergence Testing Over Spatial and Temporal Resolution Ranges}\label{sec:resolution}

Having demonstrated improving energy conservation with respect to spatial resolution, we now examine the overall convergence of the $\psi$ field configuration as the spatial resolution and timestep are augmented in a typical simulation. We initialise a simulation of two solitons diametrically opposed about a large Newtonian central potential. We run the simulation forward until the solitons become significantly tidally disrupted in their orbits, as shown in Figure \ref{fig:disruption}. 

\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 0 0 0,clip]{first_and_last}
  \caption{Visualisation of the configuration used to test $\psi$ convergence under changes to spatial and temporal resolution. In code units, two solitons of mass $m=20$ are initialised at radial distances $r=2$ from a central mass $M=1000$. The solitons are given initial velocity of $\vert v\vert=\sqrt{M/r}$ in opposing directions, corresponding to a clockwise orbit around the central mass. In code units, the total box size is 10, while the duration of the simulation is 0.25. Time runs from left to right in the figure.}
  \label{fig:disruption}
\end{figure}

To examine the sensitivity of the final $\psi$ field configuration to changes in spatial resolution, we first run this simulation at $256^3$, with the default timestep. We then re-run the simulation at resolutions ranging from $64^3$ to $320^3$, fixing the timestep in each case to be the same as the $256^3$ run. The final output arrays are then down-sampled such that we obtain five arrays of dimension $64^3$ for comparison. We arrange the array values in order of increasing density, and then analyse the deviation of the phase and the magnitude of $\psi$ from the values of the highest resolution run. The results of this process are shown in Figures \ref{fig:spatial_magnitudes} and \ref{fig:spatial_phases}. As expected, the lowest resolution run demonstrates poor convergence in both the magnitude and phase of $\psi$, with convergence increasing to acceptable levels at $256^3$.

\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 1cm 0 1.2cm,clip]{spatial_discretisation}
  \caption{Improving convergence of $\vert\psi\vert$ with increased spatial resolution for the simulation shown in Figure \ref{fig:disruption}. Field values are arranged in order of increasing magnitude of the highest resolution run from left to right.}
  \label{fig:spatial_magnitudes}
\end{figure}

\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 0.5cm 0 1.2cm,clip]{phases}
  \caption{Deviation of the phase of $\psi$ compared to the highest resolution result ($320^3$). Field values are arranged in order of increasing magnitude from left to right. A slight improvement in phase convergence can be seen for higher density regions to the right.}
  \label{fig:spatial_phases}
\end{figure}

To examine the sensitivity of the final $\psi$ field configuration to the choice of the timestep, we run the simulation at $256^3$, choosing the timestep to be 0.1, 1, 10, and 50 times the default timestep. Again we down-sample the output arrays to $64^3$, and arrange the array values in order of the $\psi$ field magnitude (as determined by the smallest timestep run). We then examine how the phase and magnitude of the field values differ in comparison to the case of 0.1 $\times$ the default timestep. The results of this analysis are shown in Figures \ref{fig:temporal_magnitudes} and \ref{fig:temporal_phases}. We notice that the deviation of the results of the default timestep from those of the timestep which is 10 times smaller are negligible, indicating that our choice of default timestep is suitably small to ensure convergence. The results remain reasonably accurate as the timestep is increased within $~10$ times the default value. At $50$ times the default value, both phase and magnitude data become significantly less accurate. 

\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 1cm 0 1cm,clip]{temporal_discretisation}
  \caption{Deviation of the magnitude of $\psi$ as compared to the simulation run with 0.1 times the default timestep. The convergence of the results for the default timestep with the results from the smaller timestep run demonstrate the suitability of the chosen default. We see relatively good convergence, particularly in high density regions, for a $<10$-fold increase above the default timestep.}
  \label{fig:temporal_magnitudes}
\end{figure}

\begin{figure}
  \includegraphics[width=1.\textwidth,trim=0 0.5cm 0 1cm,clip]{temporal_phases}
  \caption{Deviation of the phase of $\psi$ from the results of the simulation in which the timestep is set to 0.1 times the default. Again we see excellent convergence for the default timestep, and reasonable convergence for $<10$ times the default, with accuracy improving at high density regions.}
  \label{fig:temporal_phases}
\end{figure}

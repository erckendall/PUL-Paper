...
\re{do we define $\langle \rangle$? Also -- should we explain in a sentence WHY the form of the equation changes when we go to period b.c.s?}\ek{I've given slightly more info here, think this is sufficient to understand the form of 2.8}
...
The default timestep in  \PyUltraLight  is fixed with reference to the fluid interpretation of the Schr{\"o}dinger-Poisson system \cite{Hui:2016ltb}. The fluid interpretation is often used to recast the Schr{\"o}dinger-Poisson system in the form of the Madelung equations \cite{Suarez:2011yf}, such that the system can then be studied using the principles of hydrodynamics.\ek{Slightly more info and reference given here, though I don't want to go into too much detail as the specifics of the fluid representation aren't needed here.} The first step is to define:
\begin{equation}\label{eq:madelung}
    \psi\equiv\sqrt{\rho}e^{i\theta}, \quad \vec{v}\equiv\boldsymbol{\nabla}\theta.
\end{equation}
From this definition of the fluid velocity, we see that if the phase change between two adjacent grid points exceeds $\pi$,\ek{not over a timestep, but at the same point in time} the fluid will appear to move backwards across the grid. Therefore, we set the default timestep, $\Delta t$, such that fluid travelling at this maximum velocity will traverse one grid space, $\Delta x$, per timestep, or
\begin{equation}\label{eq:timestep}
    \Delta t = \frac{(\Delta x)^2}{\pi}.
\end{equation}
This is a choice, rather than a strict constraint on the timestep. We motivate this choice of timestep by recognising that if the ``fluid'' approaches velocities where the phase appears to switch direction, the configuration will also be approaching the point where the simulation grid is too coarse to fully resolve the dynamics. Hence, making the timestep much smaller than this value may offer little practical advantage. Conversely, in some cases the breakdown may occur in regions of the simulation volume of little physical interest, and the user is free to choose a larger timestep  via the 'step\textunderscore factor' parameter in the Jupyter notebook.
...
\begin{figure}
  ...
  \caption{The configuration used to test the sensitivity of solutions to the spatial and temporal resolution. Two solitons of mass $m=20$ are initialised at radial distances $r=2$ from a central mass with $M=1000$ moving in opposite directions with initial speeds $\vert v\vert=\sqrt{M/r}$, corresponding to clockwise orbits around the central mass. The  box size is 10, while the total duration is 0.25 (all quantities in code units. Time runs from left to right. \re{you could actually zoom in, to only include the active region.}\ek{I'm just concerned about misrepesenting the size of the simulation grid - I wanted to accurately represent the buffer zone, because insufficient buffer will affect the dynamics, and people trying to reproduce these results might get confused. In any case, the visual details of the density distribution aren't important here, the purpose of the figure is just to give an idea of the setup} }
  \label{fig:disruption}
\end{figure}

To examine the sensitivity of the  $\psi$ field configuration to the spatial resolution, we first run at $256^3$ with the default timestep. We then re-run  at resolutions  from $64^3$ to $320^3$ with the timestep fixed to the  $256^3$ value and downsample the final outputs to $64^3$. We  sort the resulting values by  the density at the corresponding spatial location, and plot differences in the phase and the magnitude of $\psi$ relative to the values of the $320^3$ run as shown in Figure \ref{fig:spatial} (bottom). \re{maybe this could be one combined figure with both plots.}\ek{Done.} The convergence is poor at $64^3$, but improves with resolution, to the point that there is little difference between the  $256^3$ and $320^3$ cases.

To examine the sensitivity of the the  $\psi$ field configuration to the timestep, we take the same default simulation at $256^3$, and then compare this to runs with timesteps 0.1, 10, and 50 times the default and down-sample the final output arrays to $64^3$. We sort sort the array values in order of the $\psi$ field magnitude in the run with the smallest timestep and in Figure \ref{fig:temporal} \re{maybe also combined into one full page figure?}\ek{Done.} we show the difference in the phase and magnitude of $\psi$ as a function of the timestep.  The difference between the results with the default timestep and a value 10 times smaller are negligible; and there is reasonable agreement between the default case and those with the timestep boosted by a factor of 10. However, when the timestep is increased by a factor of $50$ the accuracy of both the phase and magnitude data are significantly reduced.
...
\begin{figure}
  ...
  \caption{Top: Deviation of the phase of $\psi$ from the results of the simulation in which the timestep is set to 0.1 times the default. We see excellent convergence for the default timestep, and reasonable convergence for $<10$ times the default, with accuracy improving at high density regions. Bottom: Deviation of the magnitude of $\psi$ as compared to the simulation run with 0.1 times the default timestep. The convergence of the results for the default timestep with the results from the smaller timestep run demonstrate the suitability of the chosen default. We see relatively good convergence, particularly in high density regions, for a $<10$-fold increase above the default timestep.}
  \label{fig:temporal}
\end{figure}

\ek{added new content from here to the end of the section}
To supplement our analysis of the dependence of the field configuration on spatial and temporal resolutions, we include Figures \ref{fig:comparison} and \ref{fig:comparison2}. These figures represent the density profiles down the axis of symmetry of the initial configuration (vertical axis in Figure \ref{fig:disruption}), however rather than the profiles at $t=0$, these are the profiles after approximately half a revolution around the central potential at t=0.28 code units. At this point, the two initial solitons have become distorted due to tidal forces, but have yet to become completely disrupted. Figure \ref{fig:comparison} demonstrates the effect of increasing the timestep from 0.1 times the default value to a maximum of 50 times the default value. We see that the profiles for the default timestep and 0.1 times the default timestep are virtually indistinguishable, with 10 times the default timestep performing resonably well, with small deviations at high densities. Meanwhile, at 50 times the default timestep the results are significantly distorted. Figure \ref{fig:comparison2} demonstrates the effect of decreasing the spatial resolution from $320^3$ to $64^3$. As expected, the lowest resolution performs poorly, whereas we see high convergence of the density profile at resolutions above $192^3$.
